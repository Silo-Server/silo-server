...
func TestWorkerModesDoNotOpenArtworkStorage(t *testing.T) {
	for _, mode := range []string{"proxy", "transcode", "worker"} {
		t.Run(mode, func(t *testing.T) {
			...
			if settings.reads != 0 || deps.Artwork != nil || deps.ArtworkSigner != nil || deps.ArtworkResolver != nil {
...
func TestCatalogModesEnforceArtworkBackend(t *testing.T) {
	for _, mode := range []string{"api", "integrated"} {
		t.Run(mode, func(t *testing.T) {
			...
			if deps.Artwork == nil || deps.ArtworkBackend != "local" || deps.ArtworkSigner == nil || deps.ArtworkResolver == nil {
				t.Fatal("catalog artwork dependencies missing")
			}
		})
	}
}
