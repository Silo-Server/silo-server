...
Internals:
- A private `manifestRuntime` type (defined **in the `runtime` package**) embeds `pluginv1.UnimplementedRuntimeServer` and holds the loaded `*pluginv1.PluginManifest`. It implements `GetManifest` (returns the manifest), `Configure` (no-op `&ConfigureResponse{}`), and **`BindHostBroker` inline** — calling the same-package `SetHostBrokerID(req.GetBrokerId())` so `runtime.Host()` works for any future plugin that needs a host-proxied call.
- **Import-cycle note:** it does NOT embed `runtimedefault.Server`. `runtimedefault` imports `runtime` (its `BindHostBroker` delegates to `runtime.SetHostBrokerID`), so `runtime` importing `runtimedefault` would cycle. Because `SetHostBrokerID` is already exported *from the `runtime` package*, the inline `BindHostBroker` gets the identical broker-wiring with no `runtimedefault` dependency. `runtime` adds one new import: `pkg/pluginsdk/manifest` (for `LoadWithChecksum`) — verify this introduces no cycle (`manifest` does not import `runtime`; `go build ./...` confirms).
...
**Test** (in the `runtime` package): a `manifestRuntime` built from a test manifest returns it from `GetManifest` and an empty `ConfigureResponse` from `Configure`; assert the type satisfies `pluginv1.RuntimeServer` (compile-time `var _ pluginv1.RuntimeServer = (*manifestRuntime)(nil)`) and that `BindHostBroker` returns a non-nil `*BindHostBrokerResponse` without error. `ServeManifest` itself blocks on `plugin.Serve`, so it is exercised indirectly via the plugins' existing manifest-load tests rather than a direct unit test.
...
- **Broker wiring** fixes the latent unhooked-broker gap (`ServeManifest`'s `manifestRuntime` implements `BindHostBroker` → `SetHostBrokerID`); no current plugin calls `runtime.Host()`, so there is no behavior change today, only a removed future trap. (Implemented inline rather than via `runtimedefault.Server` to avoid a `runtime → runtimedefault → runtime` import cycle — see Component 3.)
