...
Admins can queue consistent dry runs or resumable safe purges for one library or the server. Purges retain shared and non-reconstructible revisions, transition provider and accessible sidecar artwork back to a verified source before queuing reference-aware garbage collection, and expose results through durable admin jobs. Accessible sidecars use an opaque signed direct-library route that revalidates the current catalog source and library-root confinement on every request. Scanner and library cleanup paths no longer delete artwork objects inline.
