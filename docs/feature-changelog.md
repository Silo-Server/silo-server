...
### Reach Plex history imports through reverse proxies
Plex history import now passes the advertised addresses for the selected server to Silo. The server tries a bounded set of those connections until one can return the library list, then uses that working address for the rest of the import. Profile OAuth imports connect directly to public HTTPS destinations only; they do not use an outbound HTTP proxy. Administrator-configured saved sources may still use HTTP and private LAN or container addresses. Existing clients that send only one Plex URL keep working unchanged.
