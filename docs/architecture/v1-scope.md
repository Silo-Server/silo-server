...
| The `audio` value of `effective_play_method` on `GET /api/v1/admin/sessions` | Activity reporting, [#922](https://github.com/Silo-Server/silo-server/issues/922) | Replaced before lock by `direct_stream` for video copy with audio conversion. The capability advertises the four whole-session values; the shared web admin surfaces are updated together. The raw execution method and per-stream decisions keep their meanings. This is an admin reporting change, not a native playback-plan change. |
