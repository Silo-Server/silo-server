...
| The `artwork.delivery_policy` and `artwork.url_auth` admin settings and the direct artwork-delivery mode | Artwork delivery simplification, 2026-08-26 | Resilient target-bound delivery is now the only artwork behavior, eliminating a second mode that bypassed request-time recovery. The artwork capability's `delivery_policy`, `delivery_modes`, and `automatic_recovery` fields are kept and pinned to `"resilient"`, `["api"]`, and `true` so capability-detecting clients keep working. |
| Raw-key `GET\|HEAD /api/v1/artwork/{image-type}/{shard}/{revision}/{variant}.{ext}` | Artwork delivery simplification, 2026-08-26 | The route existed only for direct local delivery and exposed the portable storage-key shape in client URLs. Target-bound `GET\|HEAD /api/v1/artwork/{capability}/{variant}` remains the single native artwork route, with request-time recovery and opaque capabilities. |
| The `artwork.local_ownership` admin setting | Artwork storage simplification, 2026-08-26 | Every local root now uses the cautious semantics formerly selected for shared mounts: bootstrap remains automatic, while a pinned missing or mismatched root refuses writes and requires an explicit rebuild. Keeping an ownership enum would expose two choices with one behavior. |
| The `artwork.seed_adoption_grace` admin setting | Artwork storage simplification, 2026-08-26 | Imported unreferenced seeds retain the established fixed 30-day adoption window before scheduled artwork-revision GC or the on-demand purge job may reclaim them. The tuning surface added lifecycle complexity without a distinct supported policy. |
