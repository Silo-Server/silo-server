...
- **Mutation retry safety is encoded.** The ledger's curated `retry_safety` field classifies
  ...
  tier-1 ported mutation rows (218 distinct operations) are classified (158
  `natural_idempotent`, 31 `unique_constraint`, 11 `domain_identity`, 9 `coalescing`, 9
  ...
  retry. Seven rows carry a `DEFECT` note where v1 gates on process-local state, fires an
  ...
  signup, playback route events); their v2 port must move the gate to shared durable state
  or add the missing unique constraint before the declared strategy holds.
