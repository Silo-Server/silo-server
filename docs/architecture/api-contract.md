...
- **Mutation retry safety is encoded.** The ledger's curated `retry_safety` field classifies
  ...
  `natural_idempotent`, 25 `unique_constraint`, 12 `domain_identity`, 9 `coalescing`, 10
  `durable_dispatch`, 57 `non_retryable`, 0 `idempotency_key`, counted per distinct
  ...
  repeat unclaimed provider work. Their ledger notes retain the required durable repair. Forty rows (39 distinct operations)
