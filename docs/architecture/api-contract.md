...
(`providers`, `user/libraries`) are `{items}` collections with string ids; the `sessions`
envelope is a limit + opaque keyset cursor page (`created_at DESC, id DESC`) of the account's
live sessions only, expired and revoked rows excluded in SQL so the listing stays bounded however
many rows retention has yet to delete (v1 keeps its unbounded all-rows list unchanged);
`policy/capability` is a capability document whose
