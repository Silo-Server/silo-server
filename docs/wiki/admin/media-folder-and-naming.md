...
Some NFO sidecars are actively read: `movie.nfo`, `tvshow.nfo`, and `<media basename>.nfo`
feed the built-in NFO metadata provider — including `<uniqueid>` provider IDs, which act as
trusted matching anchors just like `{tmdb-...}` folder tags. Season and episode NFOs and
sidecar artwork are recognized but not read yet. NFO files never change how files are
grouped; naming and layout still decide series/season/episode structure. See
[Local NFO Metadata](nfo-local-metadata.md) for the supported fields and merge behavior.
