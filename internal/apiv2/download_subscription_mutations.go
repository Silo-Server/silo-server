...
func registerDownloadSubscriptionMutations(reg *Registry) {
	create := Operation{Operation: humaOp(http.MethodPost, Prefix+"/downloads/subscriptions", "createDownloadSubscription", "downloads", "Create a monitor, or return the device's existing monitor for the series with its options unchanged and its record of deleted episodes cleared, so the next sync can register them again. Sync explicitly after receipt; do not automatically resend an uncertain create."), Class: ClassProfileScoped, ServiceBacked: true, DemoRestricted: true, RetrySafety: RetrySafetyNonRetryable}
