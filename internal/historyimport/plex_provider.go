...
// already trimmed, deduped, and capped by plexBaseURLCandidates. The slice is
// copied so a caller's later mutation cannot reorder a run's fallbacks
// mid-flight.
func NewPlexServerProvider(client *PlexClient, baseURLs []string, token string) *PlexServerProvider {
	return &PlexServerProvider{client: client, baseURLs: append([]string(nil), baseURLs...), token: token}
}
...
// loopback, link-local, and other special-use destinations. Administrator
// configured saved sources intentionally keep the unrestricted client.
