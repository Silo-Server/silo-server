...
func TestBuildSourceAlignedPlaybackManifest_CopyVideoUsesCompleteKeyframeTimeline(t *testing.T) {
	tempDir := t.TempDir()
	manifest := strings.Join([]string{
		"#EXTM3U",
		"#EXT-X-VERSION:7",
	...
	session := &TranscodeSession{
		...
		copyManifestProbe: func(context.Context, string) ([]float64, float64, error) {
			return []float64{0, 2.5, 4.1, 6.9}, 10, nil
		},
	}

	deadline := time.Now().Add(time.Second)
	var text string
	for !strings.Contains(text, "#EXT-X-PLAYLIST-TYPE:VOD") {
		got, err := session.BuildSourceAlignedPlaybackManifest("segment/", "token=test")
		if err != nil {
			t.Fatalf("BuildSourceAlignedPlaybackManifest: %v", err)
		}
		text = string(got)
		if time.Now().After(deadline) {
			t.Fatalf("complete copy manifest was not published:\n%s", text)
		}
		runtime.Gosched()
	}
	for _, want := range []string{
		...
		"#EXTINF:2.800000,",
		"#EXTINF:3.100000,",
		"segment/seg_00003.m4s?token=test",
		"#EXT-X-ENDLIST",
	} {
		if !strings.Contains(text, want) {
			t.Fatalf("complete copy manifest missing %q:\n%s", want, text)
		}
	}

	seekSeconds, ok, err := session.RestartSeekTarget(3)
	if err != nil || !ok {
		t.Fatalf("RestartSeekTarget(3) = (%v, %v, %v), want resolved future segment", seekSeconds, ok, err)
	}
	if math.Abs(seekSeconds-6.9) > 0.000001 {
		t.Fatalf("RestartSeekTarget(3) = %v, want 6.9", seekSeconds)
	}
}

func TestBuildSourceAlignedPlaybackManifest_CopyVideoFirstResponseWaitsForCompleteTimeline(t *testing.T) {
	tempDir := t.TempDir()
	manifest := strings.Join([]string{
		"#EXTM3U",
		"#EXT-X-VERSION:7",
	...
	session := &TranscodeSession{
		outputDir: tempDir,
		opts: TranscodeOpts{
			InputPath: "movie.mkv", TargetCodecVideo: "copy",
			SegmentDuration: 2, TotalDuration: 4,
		},
		copyManifestProbe: func(ctx context.Context, _ string) ([]float64, float64, error) {
			select {
			case <-time.After(100 * time.Millisecond):
				return []float64{0, 2}, 4, nil
			case <-ctx.Done():
	...
	started := time.Now()
	got, err := session.BuildSourceAlignedPlaybackManifest("segment/", "")
	if err != nil {
		t.Fatalf("BuildSourceAlignedPlaybackManifest: %v", err)
	}
	if elapsed := time.Since(started); elapsed < 75*time.Millisecond {
		t.Fatalf("manifest response returned before keyframe extraction after %v", elapsed)
	}
	if !strings.Contains(string(got), "#EXT-X-PLAYLIST-TYPE:VOD") {
		t.Fatalf("first response should contain the complete VOD manifest:\n%s", got)
	}
}

func TestCopyVideoSegmentDurationsRejectsOverflowingDuration(t *testing.T) {
	if _, err := copyVideoSegmentDurations([]float64{0}, math.MaxFloat64, 2); err == nil {
		t.Fatal("copyVideoSegmentDurations accepted a duration above the manifest segment limit")
	}
}
