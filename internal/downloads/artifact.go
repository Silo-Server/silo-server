...
// sha256(format | container | codec_video | codec_audio | resolution | audio_track_index | subtitle_burn_in).
func paramsHash(format, container, codecVideo, codecAudio, resolution string, audioTrackIndex int, subtitleBurnIn bool) string {
	sum := sha256.Sum256([]byte(fmt.Sprintf("%s|%s|%s|%s|%s|%d|%t",
		format, container, codecVideo, codecAudio, resolution, audioTrackIndex, subtitleBurnIn)))
