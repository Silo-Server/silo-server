...
func TestHandleStartPlaybackVirtualV3ReturnsHLSPlan(t *testing.T) {
	file := &models.MediaFile{ID: 43, ContentID: "movie-2", FilePath: "virtual://movie/tt0133094", Duration: 8160}
	...
	if response.PlaybackPlan.Engine != playback.EngineMedia3HLSV3 {
		t.Fatalf("engine = %s, want %s", response.PlaybackPlan.Engine, playback.EngineMedia3HLSV3)
	}
	if response.PlaybackPlan.Stream.Protocol != playback.StreamHLSV3 {
		t.Fatalf("protocol = %s, want %s", response.PlaybackPlan.Stream.Protocol, playback.StreamHLSV3)
	}
}
