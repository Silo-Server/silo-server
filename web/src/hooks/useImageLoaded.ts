 ...
 * route's exp/sig, an S3 presigned signature, or a CDN token. The server signs
 * the same key again as those age out, and each API node signs on its own, so
 * for such an image the URL without its query is the identity. Any other URL
 * may carry new bytes behind the same path, so the whole URL identifies it.
 */
...
export function useImageLoaded(url: string | undefined | null): {
  ...
  return { loaded: !!identity && loadedIdentity === identity, onLoad };
}
